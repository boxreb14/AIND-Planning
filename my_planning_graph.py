...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            acNode = PgNode_a(action)
            if acNode.prenodes.issubset(self.s_levels[level]):
                actionNodes.append(acNode)
                for sNode in self.s_levels[level]:
                    sNode.children.add(acNode)
                    acNode.parents.add(sNode)
    ...
    def add_literal_level(self, level):
        ...
        sNodes = set()
        for acNode in self.a_levels[level-1]:
            for s_node in acNode.effnodes:
                sNodes.add(s_node)
                s_node.parents.add(acNode)
                acNode.children.add(s_node)
